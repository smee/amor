...
public class GitUtility {
	 ...
	 * @return returns a {@link List} with the names of all branches of the repository.
	 */
	 ...
	 * @param refName the name of the branch, e.g. <code>refs/remotes/x</code>.
	 ...
	 * This method differs from {@link RevCommit#getParents()} since it resolves all parents recursively up to the initial commit.
	 * @param c the commit whose parents are searched.
	 * @return an {@link ArrayList} containing all Commits preceding <code>c</code>.
	 */
	 ...
	 * This means that there is no recursion and that for each child the none recursive method {@link RevCommit#getParents()} will contain <code>c</code>.
	 * @param c the commit whose children are searched.
	 * @param allRevisions a List containing all revisions of this file within the repository.
	 * @return an {@link ArrayList} containing all Commits succeeding <code>c</code>, <code>null</code> if there are no children.
	 */
	public static ArrayList<RevCommit> getDirectChildren(RevCommit c, ArrayList<FileRevision> allRevisions) {
		...
		//iterate over all revisions and add those to the child list which contain c in their parent list.
	...
	/*
	 * Helper method for #getAllChildren.
	 * This method implements the recursion which fills the list of children.
	 */
	private static ArrayList<RevCommit> getChildrenRecursive(RevCommit c, ArrayList<FileRevision> allRevisions) {
		ArrayList<RevCommit> children = new ArrayList<RevCommit>();
	 ...
	 * This method differs from {@link #getDirectChildren} since it resolves all children recursively up to the latest commit.
	 * @param c the commit whose children are searched.
	 * @param allRevisions a List containing all revisions of this file within the repository.
	 * @return an {@link ArrayList} which contains all children of the commit.<br>
	 * Therefore for each direct child commit (branch) the list contains another list which contains all its commits till the latest one.<br>
	 * if the branches are merged later there will be duplicated commits, e.g. if the latest commit merges all branches together all lists will contain the latest commit.
	 */
