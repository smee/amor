...
public class Graph<T extends Object> {
    ...
    public static class Node<T> implements Comparable<Node<T>> {
        ...
        private final Map<T, T> m_predecessors = new HashMap<T, T>();

        private final Set<Node<T>> m_neighbors = new HashSet<Node<T>>();

        public Node(final T tm) {
            m_object = tm;
        }
        public void addNeighbor(final Node<T> neighbor) {
            m_neighbors.add(neighbor);
        }

        public void addPredecessor(final T tm) {
        ...
        @Override
        public Node<T> clone() {
            final Node<T> result = new Node<T>(m_object);
            for (final T pred : m_predecessors.values()) {
        ...
        public boolean hasPredecessor(final T m) {
         ...
         */
        public boolean removePredecessor(final T o) {
            boolean result = false;

            //      dump();
            final T pred = m_predecessors.get(o);
            if (null != pred) {
        ...
        public String toString() {
            final StringBuffer sb = new StringBuffer("[Node:" + m_object);
            sb.append("  pred:");
            for (final T o : m_predecessors.values()) {
    ...
    private final Map<T, Node<T>> m_nodes = new HashMap<T, Node<T>>();
    ...
    public static void main(final String[] argv) {
        ...
        final List<String> l = g.getStrictlySortedNodes();
        ...
        ppp("GRAPH:" + g);
        {
            final List<String> predecessors = g.findPredecessors("2.1");
            assert predecessors.size() == 1;
            assert predecessors.get(0).equals("1");
        }

        {
            final List<String> predecessors = g.findPredecessors("3");
    ...
    private static void ppp(final String s) {
        if (m_verbose) {
            System.out.println("[Graph] " + s);
        }
    }

    private void addNeighbor(final T tm, final T predecessor) {
        findNode(tm).addNeighbor(findNode(predecessor));
    }

    public void addNode(final T tm) {
    ...
    public void addPredecessor(final T tm, final T predecessor) {
        final Node<T> node = findNode(tm);
        if (null == node) {
            throw new RuntimeException("Non-existing node: " + tm);
        }
        else {
    ...
    private void dumpGraph() {
        ...
        for (final Node<T> n : m_nodes.values()) {
    ...
    private void dumpSortedNodes() {
        ...
        for (final T n : m_strictlySortedNodes) {
            System.out.println("              " + n);
        }
        System.out.println("====== END SORTED NODES");
    }

    private Node<T> findNode(final T object) {
        return m_nodes.get(object);
    }

    private Node<T> findNodeWithNoPredecessors(final List<Node<T>> nodes) {
     ...
     */
    public List<T> findPredecessors(final T o) {
        // Locate the node
        final Node<T> node = findNode(o);
        if (null == node) {
        ...
        while (! queue.isEmpty()) {
            for (final T obj : getPredecessors(queue.removeFirst())) {
    ...
    public Set<T> getNeighbors(final T t) {
        final Set<T> result = new HashSet<T>();
        for (final Node<T> n : findNode(t).getNeighbors()) {
    ...
    public Set<T> getPredecessors(final T node) {
    ...
    public boolean isIndependent(final T object) {
     ...
     */
    private void removeFromNodes(final List<Node<T>> nodes, final Node<T> node) {
        nodes.remove(node);
        for (final Node<T> n : nodes) {
    ...
    public void topologicalSort() {
        ...
        // not independent.
        //
        final List<Node<T>> nodes2 = new ArrayList<Node<T>>();
        ...
        while (! nodes2.isEmpty()) {
            ...
            //
            final Node<T> node = findNodeWithNoPredecessors(nodes2);
            if (null == node) {
                final List<T> cycle = new Tarjan<T>(this, nodes2.get(0).getObject()).getCycle();
    ...
    @Override
    public String toString() {
        final StringBuffer result = new StringBuffer("[Graph ");
        for (final T node : m_nodes.keySet()) {
